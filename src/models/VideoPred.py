...
class VideoPredModel(nn.Module):
    ...
    def __init__(self, model_params, linear=True, **kwargs):
        ...
        self.linear = linear # Moving_Minist => False
    ...
    def _get_encoder_decoder(self):
        ...
        num_channels = self.model_params["num_channels"] # 3
        enc_type = self.model_params["enc_dec_type"] # DCGAN
        enc_dim = dec_dim = self.model_params["enc_dec"]["dim"] # enc_dim = dec_dim = 512
        num_filters = self.model_params["enc_dec"]["num_filters"] # num_filters = 64
        extra_deep = self.model_params["enc_dec"]["extra_deep"] # extra_deep = False
        deeper_enc = self.linear and ("SpatioTempHierarch" not in self.model_params["model_type"]) # deeper_enc = False
        if enc_type == "DCGAN":
            if deeper_enc:
